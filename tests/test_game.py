...
class TestGame(unittest.TestCase):
    def setUp(self):
        self.game = Game()
        self.game.add_character({'name': 'Warrior', 'initiative': 15})
        self.game.add_character({'name': 'Mage', 'initiative': 10})
    
    def test_add_character(self):
        # Ensure that characters are added and sorted correctly
        self.assertEqual(len(self.game.characters), 2)
        self.assertEqual(self.game.characters[0]['name'], 'Warrior')
        self.assertEqual(self.game.characters[1]['name'], 'Mage')

    def test_game_state_on_start(self):
        # Check if the game state is set to 'active' on start
        self.game.start_game()
        self.assertEqual(self.game.game_state, 'active')
    
    def test_save_and_load_game(self):
        # Test saving and loading game functionality
        self.game.start_game()
        saved_df = self.game.save_game()
        self.game.game_state = 'unknown'  # Change state to test loading function
    ...
    def test_end_game(self):
        ...
        self.game.end_game()
        self.assertEqual(self.game.game_state, 'draft')
