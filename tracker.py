...
def display_tracker():
    ...
    def update_ap_options(change):
        ...
        if selected_action:
            all_valid_ap_options = list(ap_sta_map[selected_action].keys()) if selected_action in ap_sta_map else ['1AP', '2AP', '3AP', '4AP']

            # Filtering based on current_AP
            valid_ap_options = [ap for ap in all_valid_ap_options if int(ap[0]) <= current_AP]
    ...
    def update_ap_buttons():
        ...
        valid_options = [f"{i}AP" for i in range(1, current_AP + 1)] if current_AP > 0 else []
        ...
        if not valid_options:
            ...
            ap_toggle.value = valid_options[0]  # Reset the value to the first valid option
    ...
    def undo_action(change):
        ...
        if removed_action:
            ...
            action_options = [f"[{action['ACTION']}] {action['AP']} | STA:{action['STA']} | MS:{action['MS']} | ATT:{action['ATT']} | DMG:{action['DMG']}" for action in actions.get_actions()]
